// SFMLSnake.cpp : This code comes from the "16 games" series of FamTrinli
...
void Tick()
{
    ...
    // Wrap across top, bottom, left, and right sides
    if (s[0].x > N) s[0].x = 0;  if (s[0].x < 0) s[0].x = N;
    if (s[0].y > M) s[0].y = 0;  if (s[0].y < 0) s[0].y = M;

    // We hit ourselves so chop at point of collision
    for (int i = 1; i < num; i++)
